...
const theme = createTheme({
    ...
    typography: {
        ...
        subtitle1: { // Style for the intro text
            fontSize: '1.1rem',
            lineHeight: 1.6,
            color: '#424242'
        },
        subtitle2: { // Added for form section subheadings
            fontWeight: 600,
            fontSize: '0.95rem',
            color: '#555',
        },
    ...
    components: {
        ...
        MuiCard: {
            ...
            styleOverrides: {
                root: ({ ownerState, theme }) => ({ // Use function syntax to access theme and ownerState (passed from component)
                    border: `1px solid ${ownerState.selected ? theme.palette.primary.main : alpha('#000000', 0.12)}`,
                    borderWidth: ownerState.selected ? 2 : 1, // Thicker border when selected
                    boxShadow: 'none', // Ensure no shadow on cards themselves
                    transition: theme.transitions.create(['border-color', 'box-shadow', 'background-color', 'transform'], { // Added transform
                        duration: theme.transitions.duration.short,
                    }),
                    backgroundColor: ownerState.selected ? alpha(theme.palette.primary.main, 0.05) : theme.palette.background.paper, // Subtle background tint when selected
                    '&:hover': {
                        borderColor: theme.palette.primary.light, // Indicate hover with border color change
                        backgroundColor: ownerState.selected ? alpha(theme.palette.primary.main, 0.08) : alpha('#000000', 0.02), // Slightly darker background on hover
        ...
        MuiAccordion: {
             ...
             styleOverrides: {
                 root: {
                     ...
                     '&:before': { // Remove internal separator line
                         display: 'none',
                     },
                     '&.Mui-expanded': { // Ensure margin is 0 when expanded
                         margin: 0,
                     }
                 }
             }
        ...
        MuiAccordionSummary: {
             styleOverrides: {
                 root: {
                     ...
                     '&.Mui-expanded': { // Keep minimum height when expanded
                         minHeight: 48,
                     },
                 },
                 content: {
                     '&.Mui-expanded': { // Ensure margin is consistent when expanded
                         margin: '12px 0',
                     },
                 }
             }
...
function App() {
    ...
    const runSimulation = useCallback(async (data, model) => {
        ...
        console.log('Simulating prediction with:', data, 'and model:', model?.name);

        // Basic check for required inputs
        if (!data || !model) {
            setError("Missing data or model for simulation.");
            setIsLoading(false);
            return Promise.reject("Missing data or model"); // Reject the promise
        }
        ...
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                try {
                    ...
                    // if (Math.random() < 0.1) {
                    //   throw new Error("Random simulation failure!");
                    // }

                    // Simulate prediction calculation
                    const probability = Math.floor(Math.random() * 51) + 50; // Simulate 50-100%
                    const prediction = probability >= 65 ? 'On Track' : 'At Risk'; // Example threshold
                    const result = { prediction, probability };

                    console.log('Simulation result:', result);
                    setPredictionResult(result); // Store result in state
            ...
            }, 1200); // Simulate network delay (1.2 seconds)
        });
    }, []); // No dependencies, created once

    // --- Navigation Handlers ---

    // Handles moving to the next step or running the prediction
    const handleNext = async () => {
        setError(null); // Clear previous errors
        setModelSelectionError(null);

        if (activeStep === 0) { // Moving from Input Stage
            if (inputStageRef.current) {
                const isValid = await inputStageRef.current.triggerValidation(); // Validate form first
                if (isValid) {
                    const data = inputStageRef.current.getFormData(); // Get validated data
                    setFormData(data);
                    setActiveStep((prev) => prev + 1); // Move to next step
                } else {
                    console.log("Input form validation failed");
                    // Optional: setError("Please fix the errors in the form."); // Could show a global error
                }
            }
        } else if (activeStep === 1) { // Moving from Model Selection Stage (Run Prediction)
            if (!selectedModel) {
                setModelSelectionError("Please select a model before proceeding."); // Show specific error
                return;
            }
            try {
                await runSimulation(formData, selectedModel); // Run the simulation
                setActiveStep((prev) => prev + 1); // Move to result step on success
            } catch (simError) {
                // Error state is handled within runSimulation or caught here
                console.error("Caught simulation error during transition:", simError);
                // Ensure loading is off if simulation fails
                setIsLoading(false);
                // Set a generic error message if not already set by runSimulation
                if (!error) {
    ...
    const handleBack = () => {
        ...
        setModelSelectionError(null);
        if (activeStep === 2) { // If going back from results
    ...
    // --- Component Render ---
    return (
        <ThemeProvider theme={theme}> {/* Apply the defined theme */}
            <CssBaseline /> {/* Apply baseline styles & custom scrollbar */}

            {/* Main content container with responsive top/bottom margins */}
            <Container component="main" maxWidth="lg" sx={{ mt: { xs: 2, sm: 4 }, mb: 6 }}>

                {/* --- Application Header --- */}
                <Typography variant="h4" align="center" sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', mb: 1.5 }}>
                   {/* ADDED: Icon for visual anchor */}
                   <SchoolIcon sx={{ mr: 1.5, fontSize: { xs: '2rem', sm: '2.5rem' }, color: 'primary.main' }} />
                ...
                <Box sx={{
                   ...
                   <Typography variant="subtitle1" align="center" sx={{ maxWidth: '800px', margin: '0 auto' }}>
                        A student performance prediction platform that uses Naive Bayes and Decision Tree models to analyze key factors impacting academic success.
                        It helps universities and students determine if they are on track to graduate on time, offering data-driven insights to identify those who may need additional support.
                        By providing clear forecasts, GradTrackr empowers proactive measures to ensure timely graduation.
                   </Typography>
               </Box>
                ...
                <Paper sx={{
                    ...
                    {modelSelectionError && activeStep === 1 && (
                        <Alert severity="warning" sx={{ mb: 3 }}>{modelSelectionError}</Alert>
                    )}
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 3, mb: 3 }}>
                    ...
                    <Box sx={{ minWidth: '85px' }}> {/* Ensures consistent space */}
                    ...
                    <Box>
                        ...
                        {activeStep < steps.length - 1 && (
                            <Button
                                ...
                                startIcon={activeStep === 1 && isLoading ? <CircularProgress size={20} color="inherit"/> : null}
                            >
